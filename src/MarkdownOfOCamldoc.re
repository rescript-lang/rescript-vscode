...
let convertItem = item => {
  let rec convertItem = item =>
    ...
    | `Tag(_) =>
      output_string(
        stderr,
        "Warning: Unhandled tag in ocamldoc (please tell the reason-language-server maintainers)\n",
      );
    ...
    | `Example(lang, contents) =>
      let newLang =
        String.trim(lang) == ""
          ...
          : {
            let parts =
              Str.split(Str.regexp_string(";"), String.trim(lang));
            if (List.mem("ml", parts)
                || List.mem("ocaml", parts)
                || List.mem("re", parts)
  ...
  and convertInline = item =>
    switch (item.Location_.value) {
    | `Link(href, content) =>
      Omd.Url(href, List.map(convertLink, content), "")
...
let convert = text =>
        ...
        Parser_.parse_comment(
          ...
          ~containing_definition=
            Paths.Identifier.Root(
              {Root.package: "hi", file: Page("hi"), digest: "hi"},
              "What",
            ),
  ...
  | exn => [
      Omd.Text(
        "Error (invalid syntax?) while parsing ocamldoc: "
