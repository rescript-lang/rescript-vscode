...
let maybeLog = message =>
  if (debug^) {
    Log.log("[MerlinFile]: " ++ message);
  };
...
let rec tryReduce:
  'a 'b 'e.
  (list('a), 'b, ('b, 'a) => result('b, 'e)) => result('b, 'e)
 =
  (items, current, fn) => {
    switch (items) {
    | [] => Ok(current)
    | [one, ...rest] =>
      switch (fn(current, one)) {
      | Error(e) => Error(e)
...
let calcPaths = (mname, files) => {
  ...
  | {src: None, srci: None, cmt: None, cmti: Some(cmti)} =>
    Ok(`Intf((cmti, None)))
  | {src: None, srci: None, cmt: Some(cmt), cmti: None} =>
    Ok(`Impl((cmt, None)))
  | {src: None, srci: None} =>
    Error(
      "No source files for module " ++ mname ++ " - " ++ showFiles(files),
    )
  | {src: Some(src), srci: Some(srci), cmt: Some(cmt), cmti: Some(cmti)} =>
    Ok(`IntfAndImpl((cmti, Some(srci), cmt, Some(src))))
  | {src: Some(src), srci: None, cmt: Some(cmt)} =>
    Ok(`Impl((cmt, Some(src))))
  | {src: None, srci: Some(srci), cmti: Some(cmti)} =>
    Ok(`Intf((cmti, Some(srci))))
  | _ =>
    Error(
      "Insufficient build files found for module "
      ++ mname
      ++ " - "
      ++ showFiles(files),
    )
...
let getModulesFromMerlin = (~stdlibs, base, text) => {
  ...
  maybeLog(
    Printf.sprintf(
      "Local %d, Deps %d for %s/.merlin",
      List.length(localSource),
      List.length(depSource),
      base,
    ),
  );
    ...
    let rec loop = () =>
      ...
      | s =>
        if (s == "####{BSB GENERATED: NO EDIT") {
          switch (input_line(ic)) {
          | exception End_of_file =>
            Error("Bsb merlin comment not ended correctly")
          | backendLine =>
