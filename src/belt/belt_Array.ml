...
let getExn arr i =
  if Pervasives.not ((i >= 0) && (i < (length arr)))
  then Js.Exn.raiseError "File \"\", line 37, characters 6-12";
  getUnsafe arr i
let set arr i v =
  if (i >= 0) && (i < (length arr)) then (setUnsafe arr i v; true) else false
let setExn arr i v =
  if Pervasives.not ((i >= 0) && (i < (length arr)))
  then Js.Exn.raiseError "File \"\", line 43, characters 4-10";
  setUnsafe arr i v
...
let copy a =
  ...
  for i = 0 to l - 1 do setUnsafe v i (getUnsafe a i) done; v
let swapUnsafe xs i j =
  let tmp = getUnsafe xs i in
  setUnsafe xs i (getUnsafe xs j); setUnsafe xs j tmp
let shuffleInPlace xs =
  let len = length xs in
  for i = 0 to len - 1 do swapUnsafe xs i (Js_math.random_int i len) done
let shuffle xs = let result = copy xs in shuffleInPlace result; result
let reverseAux xs ofs len =
  for i = 0 to (len / 2) - 1 do
    swapUnsafe xs (ofs + i) (((ofs + len) - i) - 1)
  done
let reverseInPlace xs = let len = length xs in reverseAux xs 0 len
let make l f = if l <= 0 then [||] else (let res = Array.make l f in res)
let reverse xs =
  let len = length xs in
  let result =
    if len > 0 then makeUninitializedUnsafe len (getUnsafe xs 0) else [||] in
  for i = 0 to len - 1 do setUnsafe result i (getUnsafe xs ((len - 1) - i))
  done;
  result
let makeByU l f =
  if l <= 0
  then [||]
    ...
    (let res = if l > 0 then makeUninitializedUnsafe l (f 0) else [||] in
     for i = 0 to l - 1 do setUnsafe res i (f i) done; res)
let makeBy l f = makeByU l (fun a  -> f a)
let makeByAndShuffleU l f = let u = makeByU l f in shuffleInPlace u; u
...
let range start finish =
  ...
  then [||]
    ...
    (let arr = makeUninitializedUnsafe (cut + 1) 0 in
     for i = 0 to cut do setUnsafe arr i (start + i) done; arr)
let rangeBy start finish ~step  =
  ...
  then [||]
    ...
    (let nb = (cut / step) + 1 in
     ...
     for i = 0 to nb - 1 do (setUnsafe arr i (!cur); cur := ((!cur) + step))
     done;
     arr)
let zip xs ys =
  ...
  let s =
    if len > 0
    then makeUninitializedUnsafe len ((getUnsafe xs 0), (getUnsafe ys 0))
    else [||] in
  for i = 0 to len - 1 do setUnsafe s i ((getUnsafe xs i), (getUnsafe ys i))
  done;
  s
let zipByU xs ys f =
  let (lenx,leny) = ((length xs), (length ys)) in
  ...
  let s =
    if len > 0
    then makeUninitializedUnsafe len (f (getUnsafe xs 0) (getUnsafe ys 0))
    else [||] in
  for i = 0 to len - 1 do setUnsafe s i (f (getUnsafe xs i) (getUnsafe ys i))
  done;
  s
let zipBy xs ys f = zipByU xs ys (fun a  -> fun b  -> f a b)
let concat a1 a2 =
  let l1 = length a1 in
  let l2 = length a2 in
  let a1a2 =
    if l1 > 0
    then makeUninitializedUnsafe (l1 + l2) (getUnsafe a1 0)
    else [||] in
  for i = 0 to l1 - 1 do setUnsafe a1a2 i (getUnsafe a1 i) done;
  for i = 0 to l2 - 1 do setUnsafe a1a2 (l1 + i) (getUnsafe a2 i) done;
  a1a2
let concatMany arrs =
  ...
  for i = 0 to lenArrs - 1 do
    (let len = length (getUnsafe arrs i) in
     ...
     if (len > 0) && ((!firstArrWithLengthMoreThanZero) = None)
     then firstArrWithLengthMoreThanZero := (Some (getUnsafe arrs i)))
  done;
  (match !firstArrWithLengthMoreThanZero with
   | None  -> [||]
   | Some firstArr ->
       let result =
         makeUninitializedUnsafe (!totalLen) (getUnsafe firstArr 0) in
       (totalLen := 0;
        for j = 0 to lenArrs - 1 do
          (let cur = getUnsafe arrs j in
           for k = 0 to (length cur) - 1 do
             setUnsafe result (!totalLen) (getUnsafe cur k); incr totalLen
           done)
        done;
        result))
  ...
  then [||]
    ...
    (let lena = length a in
     ...
     then [||]
       ...
       (let result =
          if lena > 0
          then makeUninitializedUnsafe copyLength (getUnsafe a 0)
          else [||] in
...
let fill a ~offset  ~len  v =
  ...
  then
    ...
    (if fillLength > 0
     then for i = ofs to (ofs + fillLength) - 1 do setUnsafe a i v done)
...
let forEachU a f = for i = 0 to (length a) - 1 do f (getUnsafe a i) done
let forEach a f = forEachU a (fun a  -> f a)
let mapU a f =
  let l = length a in
  let r =
    if l > 0 then makeUninitializedUnsafe l (f (getUnsafe a 0)) else [||] in
  for i = 0 to l - 1 do setUnsafe r i (f (getUnsafe a i)) done; r
...
let keepU a f =
  ...
  let j = ref 0 in
  for i = 0 to l - 1 do
    (let v = getUnsafe a i in if f v then (setUnsafe r (!j) v; incr j))
  done;
     ...
     | Some v ->
         let r =
           ...
           | None  ->
               ...
               (r := (Some newr); newr)
           | Some r -> r in
         (setUnsafe r (!j) v; incr j))
  done;
  (match !r with | None  -> [||] | Some r -> truncateToLengthUnsafe r (!j))
let keepMap a f = keepMapU a (fun a  -> f a)
let forEachWithIndexU a f =
  for i = 0 to (length a) - 1 do f i (getUnsafe a i) done
let forEachWithIndex a f = forEachWithIndexU a (fun a  -> fun b  -> f a b)
let mapWithIndexU a f =
  let l = length a in
  let r =
    if l > 0 then makeUninitializedUnsafe l (f 0 (getUnsafe a 0)) else [||] in
  for i = 0 to l - 1 do setUnsafe r i (f i (getUnsafe a i)) done; r
let mapWithIndex a f = mapWithIndexU a (fun a  -> fun b  -> f a b)
let reduceU a x f =
  let r = ref x in
  for i = 0 to (length a) - 1 do r := (f (!r) (getUnsafe a i)) done; !r
let reduce a x f = reduceU a x (fun a  -> fun b  -> f a b)
let reduceReverseU a x f =
  let r = ref x in
  for i = (length a) - 1 downto 0 do r := (f (!r) (getUnsafe a i)) done; !r
let reduceReverse a x f = reduceReverseU a x (fun a  -> fun b  -> f a b)
let reduceReverse2U a b x f =
  let r = ref x in
  let len = min (length a) (length b) in
  for i = len - 1 downto 0 do r := (f (!r) (getUnsafe a i) (getUnsafe b i))
  done;
  !r
let reduceReverse2 a b x f =
  reduceReverse2U a b x (fun a  -> fun b  -> fun c  -> f a b c)
let rec everyAux arr i b len =
  if i = len
  then true
  else if b (getUnsafe arr i) then everyAux arr (i + 1) b len else false
let rec someAux arr i b len =
  if i = len
  then false
  else if b (getUnsafe arr i) then true else someAux arr (i + 1) b len
let everyU arr b = let len = length arr in everyAux arr 0 b len
let every arr f = everyU arr (fun b  -> f b)
let someU arr b = let len = length arr in someAux arr 0 b len
let some arr f = someU arr (fun b  -> f b)
let rec everyAux2 arr1 arr2 i b len =
  if i = len
  then true
  else
    if b (getUnsafe arr1 i) (getUnsafe arr2 i)
    then everyAux2 arr1 arr2 (i + 1) b len
    else false
...
let every2 a b p = every2U a b (fun a  -> fun b  -> p a b)
let some2U a b p = someAux2 a b 0 p (min (length a) (length b))
let some2 a b p = some2U a b (fun a  -> fun b  -> p a b)
let eqU a b p =
  let lena = length a in
  let lenb = length b in
  if lena = lenb then everyAux2 a b 0 p lena else false
let eq a b p = eqU a b (fun a  -> fun b  -> p a b)
...
let cmpU a b p =
  ...
  if lena > lenb
  then 1
  else if lena < lenb then (-1) else everyCmpAux2 a b 0 p lena
let cmp a b p = cmpU a b (fun a  -> fun b  -> p a b)
let partitionU a f =
  let l = length a in
  let i = ref 0 in
  let j = ref 0 in
  ...
  for ii = 0 to l - 1 do
    (let v = getUnsafe a ii in
     if f v
     then (setUnsafe a1 (!i) v; incr i)
     else (setUnsafe a2 (!j) v; incr j))
  done;
...
let unzip a =
  ...
  for i = 0 to l - 1 do
    (let (v1,v2) = getUnsafe a i in setUnsafe a1 i v1; setUnsafe a2 i v2)
  done;
  (a1, a2)